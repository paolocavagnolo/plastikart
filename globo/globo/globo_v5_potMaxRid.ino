...
void ondaSecca(unsigned long minuti) {
  ...
  while ((millis() - durata) < fine){
    ...
    for (uint8_t j=0; j<random(3,6); j++) {
    ...
    if ((millis() - shock) > random(15000,20000)) {
      shock = millis();
      acceso();
      delay(1000);
      spegni();
...
void acceso() {
  ...
  digitalWrite(relays[2],0);
  digitalWrite(relays[3],0);
  dmx_master.setChannelValue(1, potMax);
  dmx_master.setChannelValue(2, potMax);
  dmx_master.setChannelValue(3, potMax);
  dmx_master.setChannelValue(4, potMax);
